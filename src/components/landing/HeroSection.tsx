...
function TicketRow({ model, issue, status, statusColor, delay }: {
  ...
  return (
    <motion.div
      ...
      className="flex items-center justify-between py-3 border-b border-black/5 last:border-0 group">
  ...
  return (
    <>
      ...
      <section className="relative bg-[#F8F7F4] overflow-hidden">
        ...
        {/* ── Top accent line ── */}
        <div className="absolute top-0 left-0 right-0 h-0.75 bg-linear-to-r from-transparent via-slate-300 to-transparent" />

        {/* ── Navbar ── */}
        <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 md:px-14 py-5 border-b border-black/7 bg-[#F8F7F4]/95"
        ...
        {/* ── Hero body ── */}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            {/* Glow */}
            <div className="absolute -inset-8 bg-linear-to-br from-slate-200/50 via-transparent to-slate-100/30 rounded-3xl blur-2xl" />

            {/* Main card */}
            <div className="relative bg-white rounded-2xl border border-black/8 shadow-2xl overflow-hidden">

              {/* Window bar */}
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-black/6 bg-slate-50/60">
                <div className="flex items-center gap-1.5">
                  <div className="h-3 w-3 rounded-full bg-red-400/70" />
                  <div className="h-3 w-3 rounded-full bg-amber-400/70" />
                  <div className="h-3 w-3 rounded-full bg-emerald-400/70" />
                </div>
                <div className="flex items-center gap-2 bg-white border border-black/7 rounded-md px-3 py-1">
              ...
              <div className="flex">
                {/* Sidebar */}
                <div className="w-44 border-r border-black/5 p-3 bg-slate-50/40">
                  ...
                  {[
                    ...
                    <div key={label}
                      className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg mb-0.5 cursor-default ${active ? "bg-white shadow-sm border border-black/6" : "hover:bg-white/60"}`}>
                ...
                <div className="flex-1 p-4 min-w-0">
                  ...
                  <div className="grid grid-cols-3 gap-2.5 mb-4">
                    {[
                      ...
                      <div key={k.label} className={`${k.bg} rounded-xl p-3 border border-black/4`}>
                  ...
                  {/* Ticket list */}
                  <div className="bg-slate-50/60 rounded-xl border border-black/5 px-3 pt-2 pb-1">
                  ...
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    className="mt-2.5 bg-linear-to-r from-violet-50 to-indigo-50 border border-violet-100 rounded-xl p-3">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="absolute -top-4 -right-4 bg-white border border-black/8 rounded-xl px-3 py-2 shadow-lg flex items-center gap-2">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="absolute -bottom-4 -left-4 bg-white border border-black/8 rounded-xl px-3 py-2 shadow-lg flex items-center gap-2">
