...
export default function InvoiceBuilder({ draft, saving, onChange, onSave, onCancel }: {
  ...
  return <section className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,.7fr)]">
    <div className="rounded-2xl border border-[#dedbd8] bg-white p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#006a63]">Invoice builder</p><h2 className="mt-1 text-xl font-bold">Ticket EV-{String(draft.ticketId).padStart(4, '0')}</h2></div><button onClick={onCancel} aria-label="Close invoice builder" className="rounded-lg border border-[#dedbd8] p-2 text-[#61636a] hover:bg-[#f5f3f1]"><X size={17}/></button></div>

      <div className="mt-6 space-y-4">
        <div><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">Shop name<input className={inputClass} value={draft.shopName} onChange={(e) => onChange({ shopName: e.target.value })}/></label></div>
        <div><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">Shop address<textarea className={`${inputClass} min-h-16 resize-y`} value={draft.shopAddress} onChange={(e) => onChange({ shopAddress: e.target.value })}/></label></div>
        <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">GST number (optional)<input className={inputClass} value={draft.gstNumber} onChange={(e) => onChange({ gstNumber: e.target.value })}/></label></div>
      </div>
      ...
      <div className="mt-6"><div className="flex items-center gap-2"><FileText size={17} className="text-[#006a63]"/><h3 className="font-bold">Repair summary</h3></div><p className="mt-2 rounded-xl border border-[#ebe8e5] bg-[#fbfaf9] p-4 text-sm leading-6 text-[#61636a]">{draft.repairSummary || 'No repair summary was recorded.'}</p></div>
      <div className="mt-6"><div className="flex items-center gap-2"><Package size={17} className="text-[#006a63]"/><h3 className="font-bold">Parts used</h3></div><div className="mt-3 overflow-hidden rounded-xl border border-[#e5e2df]"><div className="grid grid-cols-[1fr_42px_90px] bg-[#f5f3f1] px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-[#75777d]"><span>Part</span><span>Qty</span><span className="text-right">Amount</span></div>{draft.items.length ? draft.items.map((item, index) => <div key={`${item.sku}-${index}`} className="grid grid-cols-[1fr_42px_90px] items-center border-t border-[#eeeae7] px-4 py-3 text-sm"><span><b>{item.name}</b><small className="block font-mono text-[10px] text-[#8a8b90]">{item.sku}</small></span><span>{item.qty}</span><span className="text-right font-semibold">{formatCurrency(item.qty * item.price)}</span></div>) : <p className="p-4 text-sm text-[#75777d]">No parts were logged for this repair.</p>}</div></div>

      <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">Labor charge<input className={inputClass} type="number" min="0" value={draft.laborCharge} onChange={(e) => onChange({ laborCharge: Number(e.target.value) })}/></label><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">Tax<input className={inputClass} type="number" min="0" value={draft.tax} onChange={(e) => onChange({ tax: Number(e.target.value) })}/></label></div>

      <div className="mt-4"><label className="text-xs font-bold uppercase tracking-wider text-[#61636a]">Discount<input className={inputClass} type="number" min="0" max={totals.subtotal + totals.labor} value={draft.discount} onChange={(e) => onChange({ discount: Number(e.target.value) })}/></label></div>
